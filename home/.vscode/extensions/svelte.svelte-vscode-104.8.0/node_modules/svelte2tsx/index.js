...
function handleSlot(htmlx, str, slotEl, component, slotName, ifScope, templateScope) {
    ...
    str.appendRight(slotDefInsertionPoint, `} = ${getSingleSlotDef(component, slotName)}` + `;${ifScope.addPossibleIfCondition()}<>`);
...
const createClassAccessor = (name) => `\n    get ${name}() { return render().props.${name} }` +
    `\n    /**accessor*/\n    set ${name}(_) {}`;
const createClassAccessors = (getters, exportedNames) => {
    const accessors = [];
...
function processInstanceScriptContent(str, script, events, implicitStoreValues) {
    ...
    const propTypeAssertToUserDefined = (node) => {
        ...
        const handleTypeAssertion = (declaration) => {
            ...
            const jsDocType = ts__namespace.getJSDocType(declaration);
            const type = tsType || jsDocType;
            if (!ts__namespace.isIdentifier(identifier) ||
                (!type &&
                    // Edge case: TS infers `export let bla = false` to type `false`.
                    // prevent that by adding the any-wrap in this case, too.
                    ![ts__namespace.SyntaxKind.FalseKeyword, ts__namespace.SyntaxKind.TrueKeyword].includes((_a = declaration.initializer) === null || _a === void 0 ? void 0 : _a.kind))) {
                return;
            }
        ...
        const findComma = (target) => target.getChildren().filter((child) => child.kind === ts__namespace.SyntaxKind.CommaToken);
        const splitDeclaration = () => {
            const commas = node
                .getChildren()
                .filter((child) => child.kind === ts__namespace.SyntaxKind.SyntaxList)
                .map(findComma)
    ...
    const handleExportedVariableDeclarationList = (list) => {
        ts__namespace.forEachChild(list, (node) => {
            if (ts__namespace.isVariableDeclaration(node)) {
                if (ts__namespace.isIdentifier(node.name)) {
                    exportedNames.addExport(node.name, node.name, node.type, !node.initializer);
                }
                else if (ts__namespace.isObjectBindingPattern(node.name) ||
                    ts__namespace.isArrayBindingPattern(node.name)) {
                    ts__namespace.forEachChild(node.name, (element) => {
    ...
    const wrapExpressionWithInvalidate = (expression) => {
        ...
        // () => ({})
        if (ts__namespace.isObjectLiteralExpression(expression)) {
    ...
    const walk = (node, parent) => {
        ...
        if (ts__namespace.isInterfaceDeclaration(node) && node.name.text === 'ComponentEvents') {
            events.setComponentEventsInterface(node);
        }
        if (ts__namespace.isVariableStatement(node)) {
            const exportModifier = findExportKeyword(node);
            if (exportModifier) {
                const isLet = node.declarationList.flags === ts__namespace.NodeFlags.Let;
                const isConst = node.declarationList.flags === ts__namespace.NodeFlags.Const;
                ...
                if (isLet) {
                    ...
                    node.declarationList.forEachChild((n) => {
                        if (ts__namespace.isVariableDeclaration(n) && ts__namespace.isIdentifier(n.name)) {
                            addGetter(n.name);
                        }
                    });
                }
        ...
        if (ts__namespace.isExportDeclaration(node)) {
            ...
            if (ts__namespace.isNamedExports(exportClause)) {
...
function resolveImplicitStores(htmlx, script, implicitStoreValues, str) {
    const scriptContent = htmlx.substring(script.content.start, script.content.end);
...
function addComponentExport({ str, uses$$propsOr$$restProps, strictMode, strictEvents, isTsFile, getters, usesAccessors, exportedNames, fileName, componentDocumentation }) {
