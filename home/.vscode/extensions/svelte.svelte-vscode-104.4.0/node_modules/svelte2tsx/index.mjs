...
function handleAnimateDirective(htmlx, str, attr) {
    ...
    str.overwrite(htmlx.indexOf(':', attr.start) + 1 + `${attr.name}`.length, attr.expression.start, '(__sveltets_ElementNode,__sveltets_AnimationMove,(');
    str.appendLeft(attr.expression.end, ')))');
...
function handleTransitionDirective(htmlx, str, attr) {
    ...
    str.overwrite(htmlx.indexOf(':', attr.start) + 1 + `${attr.name}`.length, attr.expression.start, '(__sveltets_ElementNode,(');
    str.appendLeft(attr.expression.end, ')))');
    if (isQuote(htmlx[attr.end - 1])) {
        str.remove(attr.end - 1, attr.end);
    }
